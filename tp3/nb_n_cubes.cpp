...
int NCUBES;        /* cantidad de cubos=NBINS^N_IN*/
...
struct HistogramKD{
  ...
  vector<int> freq;
  HistogramKD() {}
  HistogramKD(const vector<int> &points): low(N_IN, numeric_limits<double>::max()), high(N_IN,  numeric_limits<double>::lowest()){    
    for(auto &p:points){
      for(int i=0; i<N_IN; i++){
        low[i]=min(low[i], data[p][i]);
        high[i]=max(high[i], data[p][i]);
      }
    }
    total = points.size();
    freq = vector<int>(NCUBES, 0);
    for (auto &p: points){
      //~ printf("NCUBES: %d\n", NCUBES);
      //~ printf("which(data[p]): %d\n", which(data[p]));
      freq[which(data[p])]++;
    }
  }
  int which(double x, int feature)
  {
    if (abs(x-high[feature]) < 1e-9) /* caso especial */
        return NBINS - 1;
    return (x-low[feature]) / ((high[feature]-low[feature])/NBINS);
  }
  int which(double *x){
    int p=0;
    int accum=1;
...
/*ALLOCAR ESPACIO PARA LAS MATRICES DEL ALGORITMO*/
  ...
  if(NBINS==0){
    ...
    histograms=(HistogramKD *)calloc(N_Class,sizeof(HistogramKD));
...
int arquitec(char *filename){
  ...
  if(fscanf(b,"%d",&NBINS)!=1)
    ...
    NCUBES=1;
    for(int i=0; i<N_IN; i++) NCUBES*=NBINS;
  }
  fclose(b);
...
int output(double *input){
  ...
  for(k=0;k<N_Class;k++) {
    ...
    if(NBINS==0)
      ...
      prob_de_clase += log( histograms[k].prob(input));
...
int train(char *filename){
  ...
  if(NBINS==0){
    ...
    for(j=0;j<N_IN;j++){
      vector<int> tmps[N_Class];
      for(i=0;i<PR;i++){
        int clase = round(data[seq[i]][N_IN]);
        tmps[clase].push_back(seq[i]);        
      }
      for(i=0;i<N_Class;i++)
        histograms[i] = HistogramKD(tmps[i]);
    }
  }
