...
int arquitec(char *filename){
  ...
  /* Martin: Calculo PTOT y PTEST automáticamente */
  sprintf(filepat,"%s.data",filename);
  PTOT = countLines(filepat);
  sprintf(filepat,"%s.test",filename);
  PTEST = countLines(filepat);
  
  /* Martin: Calculo PR si es un porcentaje*/
  if(tmpPR>1+1e-9) PR=tmpPR;
  else PR=round(PTOT*tmpPR);
  ...
  /* Semilla para la funcion rand() */
  if(SEED<0) srand(clock());    /* Martin: Uso clock() para mejor seed */
  else{
...
int read_data(char *filename){
  ...
  for(k=0;k<PTOT;k++){
     ...
     int clase;
     fscanf(fpat,"%d",&clase);
     if(CONTROL>1) printf("%d\t",clase);
     separador=getc(fpat);
     if(separador!=',') ungetc(separador,fpat);
 	 for(i=1;i<=N3;i++)
	   data[k][N1+i]=0.1;
     data[k][N1+1+clase]=0.9;
  }
  fclose(fpat);
  for(k=0;k<PTOT;k++){
	 data[k][0]=-1.0;
...
float propagar(float **S,int pat_ini,int pat_fin,int usar_seq){
  ...
  for (patron=pat_ini; patron < pat_fin; patron ++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    if(N3==1){
        if((x3[1]-0.5)*(S[nu][N1+1]-0.5)<0.) discrete_error+=1.0;    /*calcula error discreto para una sola salida - problema binario*/
    }
    else{
        int mx=argmax(x3+1, N3);
        int mxOk=argmax(S[nu]+N1+1, N3);
        if(mx!=mxOk) discrete_error+=1.0;
    }
...
int train(char *filename){
  ...
  for (iter=1; iter<=ITER; iter++) {
    ...
    if ((iter/NERROR)*NERROR == iter) {
      ...
      fprintf(ferror,"%d\t%f\t%f\t%f\t%f\t",iter,mse,mse_train,mse_valid,mse_test);
